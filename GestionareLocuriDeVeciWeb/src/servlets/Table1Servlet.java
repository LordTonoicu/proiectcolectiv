...
public class Table1Servlet extends HttpServlet {
	...
	private void exportExcel(HttpServletRequest request,
			...
			ArrayList<InregRegAnualDeProgrInmormantari> registru) throws IOException {
		response.setContentType("application/vnd.ms-excel");
		response.setHeader("Content-Disposition",
				"attachment; filename=registru1.xls");
		...
		try {
			...
			for (int i = 1; i < registru.size() + 1; i++) {
				sheet.addCell(new Label(0, i, registru.get(i-1).getNumeDecedat()));
				sheet.addCell(new Label(1, i, registru.get(i-1).getPrenumeDecedat()));
				sheet.addCell(new Label(2, i, registru.get(i-1).getCimitir()));
				sheet.addCell(new Label(3, i, registru.get(i - 1).getParcela()));
				sheet.addCell(new Label(4, i, String.valueOf(registru.get(i - 1).getNrLoc())));
				sheet.addCell(new Label(5, i, String.valueOf(registru.get(i - 1).getDataInmormantarii())));
			}
